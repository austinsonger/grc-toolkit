...
2. **For NIST or FedRAMP lookups, use MCP tools first** for authoritative control data:
   - **Preferred (MCP)**: Use `grc_get_oscal_control` with `catalog="nist"` or `catalog="fedramp"` and the normalized control ID (e.g., `ac-2`, `ac-2.1`). This returns the full control with statement, params, assessment objectives, assessment methods, and enhancements in one call.
   - For keyword searches, use `grc_search_oscal` with `catalog="nist"` or `catalog="fedramp"` and the keyword.
   - For cross-framework mapping, use `grc_map_control` to find equivalents in other frameworks.
   - **Fallback (direct file reading)**: If MCP tools are unavailable, determine the family ID from the control ID (e.g., `AC-2` → family `ac`, `IA-5(1)` → family `ia`) and read the OSCAL family JSON directly:
     - For `nist`: read `grc-pro/knowledge/oscal/nist-800-53-rev5/{family}.json`
     - For `fedramp`: read `grc-pro/knowledge/oscal/fedramp-moderate-rev5/{family}.json`
   ...
   - Then read the markdown framework file from `grc-pro/knowledge/frameworks/` for cross-framework mapping, audit context, and narrative guidance

3. **For all other frameworks**, read the appropriate framework reference file from `grc-pro/knowledge/frameworks/`
