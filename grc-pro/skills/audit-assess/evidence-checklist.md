...
2. **Use MCP tools first** for authoritative assessment procedures:
   - **Preferred (MCP)**: Use `grc_get_oscal_control` with `catalog="nist"` or `catalog="fedramp"` and the normalized control ID. The response includes assessment objectives and methods (EXAMINE, INTERVIEW, TEST) which map directly to the evidence checklist categories.
...
3. **Read additional reference files**:
   - Framework file from `grc-pro/knowledge/frameworks/`
   - Audit file from `grc-pro/knowledge/audits/` (if audit-type specified)
   - Narrative quality criteria from `grc-pro/knowledge/audits/narrative-quality-criteria.md` (for evidence expectations)
