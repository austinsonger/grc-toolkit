...
2. **Read the appropriate reference files**:
   - `grc-pro/knowledge/frameworks/oscal-reference.md` for OSCAL structure and concepts
   - `grc-pro/knowledge/audits/document-section-requirements.md` for SSP section requirements (for readiness topic)
   - **Preferred (MCP)**: Use `grc_get_oscal_control` to retrieve live OSCAL control examples (e.g., `catalog="nist"`, `control_id="at-1"` for a small example). Use `grc_search_oscal` to find controls relevant to the user's topic.
   - **Fallback**: Read `grc-pro/knowledge/oscal/nist-800-53-rev5/{family}.json` and `grc-pro/knowledge/oscal/fedramp-moderate-rev5/{family}.json` directly for live OSCAL data examples
