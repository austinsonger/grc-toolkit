...
2. **Use MCP tools first** for the complete control list and parameter definitions:
   - **Preferred (MCP)**: Use `grc_get_oscal_control` with `catalog="nist"` or `catalog="fedramp"` for each control in the family. The response includes statement, parameters (ODPs), assessment objectives (to ensure narrative coverage), and enhancements.
   - For listing all controls in a family, use `grc_search_oscal` with the family prefix as keyword.
   - **Fallback (direct file reading)**: If MCP tools are unavailable, read the OSCAL family JSON using the Read tool:
...
3. **Read the framework reference file** from `grc-pro/knowledge/frameworks/` for additional narrative context, audit expectations, and cross-framework guidance.
