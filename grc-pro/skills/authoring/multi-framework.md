...
2. **Read the appropriate reference files**:
   - Framework files for each specified framework from `grc-pro/knowledge/frameworks/`
   - Mapping files from `grc-pro/knowledge/mappings/` for each framework pair (chaining through NIST)
   - `grc-pro/knowledge/mappings/cross-framework-matrix.md` for high-level family mapping
